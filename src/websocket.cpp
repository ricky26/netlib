...
namespace netlib
{
	...
	websocket::frame::frame(netlib::websocket::opcode _type, int _flags,
							std::string const& _data, uint32_t _mask)
		: mOpCode(_type), mData(_data), mMask(_mask), mFlags(_flags)
	{
	}

	bool websocket::frame::valid() const
	{
		return mOpCode != 0;
	}

	websocket::opcode websocket::frame::opcode() const
	{
		return mOpCode;
	}

	int websocket::frame::flags() const
	{
		return mFlags;
	}

	std::string websocket::frame::data() const
	{
		return mData;
	}

	uint32_t websocket::frame::mask() const
	{
		return mMask;
	}
